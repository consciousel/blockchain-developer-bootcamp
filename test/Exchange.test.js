import { ether, tokens, EVM_REVERT, INVALID_ADD_TO, INVALID_ADD_TOKEN, INVALID_ADD_SPENDER, ETHER_ADDRESS } from './helpers'
...
contract('Exchange', ([deployer, feeAccount, user1, user2]) => {
	...
	describe('Withdrawing tokens', () => {
		...
		describe('failure', () => {
			it('rejects Ether deposits', async () => {
				//Attempt to deposit Ether instead of approved token  
				await exchange.depositToken(ETHER_ADDRESS, tokens(10), { from: user1 }).should.be.rejectedWith(EVM_REVERT)
			})
		...
		describe('cancelling orders', async () => {
			...
			describe('failure', async () => {
				...
				it('rejects unauthorized cancelations', async () => {
					//Try to cancel the order from another user's account
